@section('title')
    <x-pageTitle current="{{ __('dash.hospitals') }}">
        <x-breadcrumb title="{{ __('dash.home') }}" route="{{ route('dashboard.home') }}" />
        <x-breadcrumb title="{{ __('dash.hospitals') }}" active />
    </x-pageTitle>
@endsection

<div>
    <x-table :columns="['logo', 'category']">
        @forelse ($hospitals as $hos)
            <tr Role="row" class="odd">
                <td>{{ $loop->index + 1 }}</td>
                <td>
                    <img style="width:200px;height: 100px" src="{{ asset($hos->image) }}">
                </td>
                <td>
                    {{ $hos->category['title_'.lang()] }}
                </td>
                <td>
                    <a wire:click="openEditModal({{ $hos->id }})" type="button" data-bs-toggle="modal" data-bs-target="#editModal">
                        <i class="text-primary lni lni-pencil mr-10"></i>
                    </a>
                    <a wire:click="openDeleteModal({{ $hos->id }})" type="button" data-bs-toggle="modal" data-bs-target="#deleteModal">
    ...
    <x-modal wireAction="store" id="addModal" title="{{ __('dash.add') }}" type="primary">
        <div class="row">
            <x-form.select name="category" label="{{ __('dash.category') }}">
                @foreach ($categories as $category)
                    <x-form.option name="{{ $category['title_'.lang()] }}" value="{{ $category->id }}"></x-form.option>
                @endforeach
            </x-form.select>
