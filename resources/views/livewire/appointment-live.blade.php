@section('title')
    <x-pageTitle current="{{ __('dash.appointments') }}">
        <x-breadcrumb title="{{ __('dash.home') }}" route="{{ route('dashboard.home') }}" />
        <x-breadcrumb title="{{ __('dash.appointments') }}" active />
    </x-pageTitle>
@endsection

<div>
    <x-table :pagination="$contacts" :columns="['name', 'email', 'phone', 'created_at']" noCreate>
